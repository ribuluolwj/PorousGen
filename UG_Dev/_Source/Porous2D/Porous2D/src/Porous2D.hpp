...
void Porous2D::GenHexagon(NXOpen::Part* workPart)
{
    ...
    if ((std::string)(Porous2D::enumParticleShape->ValueAsString()).GetText() == "Circle")
    {
        ...
        if (Porous2D::togglePaper->Value() == true)
        {
            ...
            double yStartPoint = yZoneStart + colBoderGap + gap * sqrt(3.0) / 4.0;
        ...
        else
        {
            ...
            //----Compute colum of particle hexagon
//            int numHexagon = floor(zoneLength * zoneWidth * (1.0 - ratio) * 4.0 / PI / (diameter * diameter));
//            double particleLength = sqrt(PI * diameter * diameter / (4.0 * (1.0 - ratio)));
//            int rowHexagon = floor(zoneWidth / particleLength);
//            int columeHexagon = floor(zoneLength / particleLength - 0.5);
//            int rowHexagon = floor(sqrt(zoneLength * zoneLength * (1.0 - ratio) * 4.0 / PI / (diameter * diameter)));
//            int columeHexagon = floor(sqrt(zoneLength * zoneLength * (1.0 - ratio) * 4.0 / PI / (diameter * diameter)));
//            double rowGap = zoneWidth / (double)rowHexagon - diameter;
//            double columeGap = 2.0 * zoneLength / (2.0 * (double)columeHexagon +1.0) - diameter;
//            double rowBoderGap = (zoneWidth - rowHexagon * particleLength ) / 2.0;
//            double columeBoderGap = (zoneLength - columeSquare * particleLength) / 2.0;
 ...
 //           double extraBoderGap = (diameter + gap) * ((2.0 - sqrt(3.0)) * rowHexagon / 2.0 + (sqrt(3.0) - 1.0) / 2.0) / 2.0;
            ...
            if (Porous2D::toggleWriteFile->Value() == true)
            {
                ...
                Porous2D::outf << "->\t列数:" << rowHexagon << "\t颗粒行间距: " << gap << " (mm)" << endl;
...
//Save 3D porous media as IGES files
